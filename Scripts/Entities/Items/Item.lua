...
---------------------------------------------------------------------------------------------------  // HoO
CreateItemTable("Cannon24lb");
Cannon24lb.Properties.bMounted=1;
Cannon24lb.Properties.bUsable=1;
Cannon24lb.Properties.bPickable=0;
Cannon24lb.Properties.MountedLimits = {
	pitchMin = -22,
	pitchMax = 60,
	yaw = 70
	};
Cannon24lb.Properties.fRecoilDistance=3;
Cannon24lb.Properties.fRecoilStep=.1;
Cannon24lb.Properties.fRecoverStep=.005;

-----------------------------------------------------------------------------------------------------
function Cannon24lb:OnReset()
	self.item:SetMountedAngleLimits(self.Properties.MountedLimits.pitchMin, self.Properties.MountedLimits.pitchMax, self.Properties.MountedLimits.yaw);
	self.inUse = false
end

----------------------------------------------------------------------------------------------------
function Cannon24lb:OnSpawn()
	self:OnReset();
end

----------------------------------------------------------------------------------------------------
function Cannon24lb:OnUsed(user)
	if (user.actor:IsPlayer()) then
		return Item.OnUsed(self, user);
	end
end

function Cannon24lb:OnAIUsed(user, goalPipeID)
	if (user.OnUseMountedWeaponRequest) then
		user:OnUseMountedWeaponRequest(self.id)
		return
	end
